...
bash tests/packages.sh         # stones 标准库包回归测试（7/7 通过，十八包双侧）
...
| [10-纯卅标准库包.md](./10-纯卅标准库包.md) | 初学者 | 纯 卅 实现的 stones 十八包（算术/文本/游戏/中文数字/集合运算/统计/单位换算/进制/几何/算法 + v0.2 的数列/逻辑/密码/日期 + v0.2.1 的罗马数字/分数库/矩阵库/时长库），自举分层与 exe 内嵌说明 |
...
| [16-功能清单与路线图.md](./16-功能清单与路线图.md) | 全体/实现者 | 待办缺陷、语言增强、stones 扩充、工具链路线——每个版本从这里"领活"，含建议的 v0.3 组合 |
...
| v0.2 生态基建 | ✅ 已实现 | stones 扩充到十四包（新增 数列库/逻辑库/密码库/日期库）；VS Code 扩展 0.2.0：服务端上下文补全接入（`用 "…` 包名 / `名字.` 成员）、悬停文档、文件图标主题、市场图标、转译/REPL 命令；`editors/shared/language.json` 编辑器元数据唯一事实来源 + 多编辑器路线（JetBrains/Sublime/Vim 计划，见 editors/README.md） |
| v0.3.0 响应式转译与默认参数 | ✅ 已实现 | 格子转译支持（sahou build 的页面同样自动重算，页面.绑定/绑输入 可用）；用户函数默认参数（限不可变字面量、排末尾）；stones 十八包清理取整 hack 改用 数学.向下取整；算法库新增 汉诺塔/八皇后；LSP 服务端悬停；输出缓冲提速 |
| v0.2.1 提速与体验 | ✅ 已实现 | stones 扩充到十八包（新增 罗马数字/分数库/矩阵库/时长库）；CLI 输出改缓冲批量写出（50ms 定时冲刷兜底交互场景，循环打印系统调用减少几个数量级）；条件里误写单个 `=` 给专门双语提示；`变量.` 的 LSP 补全给出列表/字典成员方法；多行字典尾逗号/闭括号独立成行合法（v0.2 CI 期间修复） |
